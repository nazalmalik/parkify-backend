...
const exchangeRate = 296.3736;  
const amountInUSD = booking.totalPrice / exchangeRate;  

if (!amountInUSD || isNaN(amountInUSD)) {  
  return res.status(400).json({ message: 'Invalid total price conversion' });  
}  

const unitAmount = Math.max(Math.round(amountInUSD * 100), 50); // min 50 cents  
